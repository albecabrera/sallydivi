...
- **Technical accuracy:** Flag anything **invalid**, **outdated**, or **contradicted** by those sources or by the diff. Treat misleading claims about how to build or extend modules as high-severity findings.
...
- **Examples and snippets:** JSON/TS/PHP/CLI samples must be plausible and aligned with real patterns; wrong keys, shapes, or dead imports are **issue (non-blocking)** with **Concern** severity or **issue (blocking)** with **Blocker** severity when copy-paste would fail.
...
Judge whether the page **delivers on its purpose**; invalid or muddled structure here is an **issue (non-blocking)** with **Concern** severity even when individual sentences look fine.
...
- **Tutorials** (e.g. under `docs/tutorials/**`): The reader should see an **explicit path to the objective**—**goal**, **prerequisites**, **numbered or ordered steps**, expected outcomes per major step, and verification where useful. Missing steps, ambiguous “then do X,” or steps that contradict builder-5 reality are **issue (non-blocking)** with **Concern** severity or **issue (blocking)** with **Blocker** severity.
...
- **When the category is internally inconsistent:** Align the **changed or new** page with the **strongest or most common** pattern among **direct siblings**. You may note category-wide drift as a **nitpick** with **Nit** severity or **trend**, without blocking on inconsistency in distant sections of the site.
...
Examples: **Troubleshooting** (common errors after following steps), **Prerequisites**, **What you will build**, **Verification**, **Related reading**, or a closing **Summary**. Frame these as concrete suggestions (section title + what it should cover); use **nitpick** with **Nit** severity or **issue (non-blocking)** with **Concern** severity by impact, not **issue (blocking)** with **Blocker** severity unless absence causes harmful misuse.
