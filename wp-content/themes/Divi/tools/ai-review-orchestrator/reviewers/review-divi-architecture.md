...
- **Issue (non-blocking)** with **Concern** severity or **issue (blocking)** with **Blocker** severity (by impact) if JSON changed alone or conflicts with `.ts`: fix `.ts`, then run Visual Builder **`build:module-library-assets`** or the owning package’s script for that asset.
